      ...
      response.body.forEach((shopItem) {
        ...
        if(AppConstants.vendorType != VendorType.dokan) {
          ...
          if( shop.id == 1 ) {
            ...
            shop.email = Get.find<ConfigController>().settings.businessSettings.email ?? '';
            shop.address = Get.find<ConfigController>().settings.businessSettings.address ?? '';
            shop.phone = Get.find<ConfigController>().settings.businessSettings.phone ?? '';
            _shopList.add(shop);
